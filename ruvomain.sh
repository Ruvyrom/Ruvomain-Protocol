...
local raw_pkgs
raw_pkgs=$($EXEC pm list packages -u 2>/dev/null | sed 's/package://g' | tr -d '\r' | grep -v '^$' | sort)

if [[ -z "$raw_pkgs" ]]; then
echo -e "\n${RED}[!]Error:${NC} Unable to retrieve package list. Check ADB."
read -rp "Press Enter to return to main menu"
return 1
fi

echo "$raw_pkgs" | jq -R . | jq -s --arg date "$(date +%Y%m%d)" '{
name: ("Ruvomain_Snapshot_Auto_" + $date),
description: "Automatic backup generated by Ruvomain-Protocol",
author: "User",
version: "2.0",
apps: [.[] | {packageName: .}]
}' > "$output_file"

echo -e "\n${GREEN}[✓] Backuplist successfully generated in Configs/backup-restore!${NC}"
sleep 1
read -rp "Press Enter to return to main menu"
return 0
}
